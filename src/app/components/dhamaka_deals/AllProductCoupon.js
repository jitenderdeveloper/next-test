...
export async function getServerSideProps(keys) {
  ...
  const res = await fetch(
    `https://api.cashdost.com/api/store/search/q/${q}`,
    {
      cache: "force-cache",
    }
  );
  ...
  return {
    props: {
      data: data?.client_data?.coupon || {},
    },
  };
}

async function AllProductCoupon({ params }) {
  const client_key = params?.category?.split("-")?.join(" ");
  const { props } = await getServerSideProps(client_key);
  const findalDealsCoupon = props?.data?.filter((item) =>
    item.post_data.includes("success")
  );;

  // console.log("props----------------------------------", findalDealsCoupon);

  return (
    <h3>all dhamaka category</h3>
    // <>
    //   {findalDealsCoupon?.length === 0 ? (
    //     <p>Empty Data</p>
    //   ) : (
    //     <>
    //       <div className="row">
    //         {findalDealsCoupon?.map((val, ind) => {
    //           const {
    //             client,
    //             description,
    //             _id,
...
export default AllProductCoupon;
